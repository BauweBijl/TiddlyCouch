...
The user must be defined in the couchDb server first with username and password
!CouchDB logged in/out Options
!!Logged in behavior
<<option txtCouchLoginTheme>> Logged in theme
<<option chkCouchLoginReadOnly>>  Logged in readOnly
<<option chkCouchLoginShowBackstage>> Logged in Show backstage
!!Logged out behavior
<<option txtCouchLogoutTheme>> Logged out theme
<<option chkCouchLogoutReadOnly>>  Logged out readOnly
<<option chkCouchLogoutShowBackstage>> Logged out Show backstage
!The code
***/
//{{{
//configuration settings
if (config.options.txtCouchLoginTheme==undefined)
  config.options.txtCouchLoginTheme='';
if (config.options.chkCouchLoginReadOnly==undefined)
  config.options.chkCouchLoginReadOnly=false;
if (config.options.chkCouchLoginShowBackstage==undefined)
...
// Added by  PKO
// ///////////////////////////////////////////////////////////////////////////////
  // macro definition for login/logout
  config.macros.CouchDbLogin = {
    msg: "Login",
    handler: function(place,macroName,params) {
      ...
      $.couch.session({ 
        success: function (a) {
          if (a.userCtx.name){
            label="Logout";
            prompt="Close "+a.userCtx.name+" session";
            createTiddlyButton(place, label, prompt, function() {config.macros.CouchDbLogin.logout();}, null, null, this.accessKey);
          }
          else {
            label="Login";
            prompt="Open a new session";      
            createTiddlyButton(place, label, prompt, function() {config.macros.CouchDbLogin.login();}, null, null, this.accessKey);
          }
      } });
    },
    login: function() {
      $.showDialog("/_utils/dialog/_login.html", {
        submit: function(data, callback) {
          if (!config.macros.CouchDbLogin.validateUsernameAndPassword(data, callback)) return;
          config.macros.CouchDbLogin.doLogin(data.name, data.password, callback);
        }
      });
      return false;
    },
